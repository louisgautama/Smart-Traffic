...
mycursor.execute("CREATE DATABASE IF NOT EXISTS trfdb")

#Establishing database connection
db = mysql.connector.connect(user='root', password='Arcana000-', host='localhost', database='trfdb')
...
sql = ("""CREATE TABLE IF NOT EXISTS `""" +"s_"+ date_str + """` (time varchar(255), pedestrian varchar(255));""")
cursor.execute(sql)
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        #Give green sign to vehicle traffic light if default duration or internal timer goes 0
                ...
                pedReset()
                outGreen = GPIO.output(10, GPIO.HIGH)
                #Minimal time for car to move after pedestrian cross the road
                time.sleep(7)
            ...
            if ((not prev_input) and input and (len(det))):
            ...
            #Print that fact to the shell
                ...
                query = ("""INSERT INTO `"""+ "s_" + date_str + """`(time, pedestrian) VALUES (%s, %s);""")

                # Storing values in a variable
                values = (time_str, person1)
            ...
            if len(det):
                ...
                person = len(det)
                print("number of people " + str(person))
                cv2.putText(im0, "Number of People: " + str(person), (150, 25), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
